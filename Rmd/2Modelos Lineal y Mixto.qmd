...
dataMultinivel <- encuesta %>% 
  ...
  anoest = case_when(
    ...
    anoest == 99 |is.na(anoest) ~ "99"     #NS/NR ##### valida con cuidado
  ),
...
```{r, eval=TRUE}
# file.edit("../Data/modelosStan/13Multinivel_Intercepto.stan")
fitMultinivel <- cmdstan_model(stan_file = "../Data/modelosStan/13Multinivel_Intercepto.stan") 
```
...
```{r}
Xdat <- model.matrix(
  prom_Ingreso ~ sexo + edad + anoest + 
    tasa_desocupacion + discapacidad1 +
    alfabeta + rezago_escolar
    ,
  data = dataMultinivel
)
tba(head(Xdat,10))
...
Zdat <- model.matrix(prom_Ingreso ~ depto,
                     data =  dataMultinivel)
tba(head(distinct(data.frame(Zdat)),10))
```
...
```{r, eval = TRUE, message=FALSE}
model_Multinivel <- fitMultinivel$sample(
                 data = sample_data,
                 num_warmup = 1000, 
                 num_samples = 1000,
                 chains = 4,
...
```{r}
model_Multinivel$summary(variables = c("beta")) %>%
  select(variable:q95) %>% tba()
```

```{r}
model_Multinivel$summary(variables = c("gamma"))%>%
  select(variable:q95) %>% tba()

```

```{r}
model_Multinivel$summary(variables = c("sigma")) %>%
  select(variable:q95) %>% tba()
```

```{r}
mcmc_areas(model_Multinivel$draws(
  variable =   c("beta[2]","beta[3]", "beta[4]", "beta[5]"))) +
mcmc_areas(model_Multinivel$draws(
  variable = c("beta[6]","beta[7]","sigma"))) 
```

```{r,out.height="200%"}
mcmc_trace(model_Multinivel$draws(variables = "beta")) 

```

```{r,out.height="200%"}
mcmc_trace(model_Multinivel$draws("gamma")) 
```

```{r}
y_pred_B <- model_Multinivel$draws(variables = "ypred", format = "matrix")
rowsrandom <- sample(nrow(y_pred_B), 100)
y_pred2 <- y_pred_B[rowsrandom, ]
