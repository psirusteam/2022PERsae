...
```{r, echo=FALSE}
kable(Indicadores_prov,
      format = "html", digits =  4,
      caption = "Estimación para prov") %>% 
    kable_classic()
...
ShapeSAE <- read_sf("../Shape/secc_hog11.shp")%>% 
  ...
  mutate(prov = str_pad(codsec,width = 4,pad = "0"))

mapa <- tm_shape(ShapeSAE %>%
                           left_join(Indicadores_prov,  by = "prov"))
...
```{r}
Indicadores_depto <- Indicadores_censo(setdata = poststrat_df, "segm")
```
